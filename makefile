...
openshift:
	...
	cat ./scripts/postinstall-master.sh | ssh -A ec2-user@$$(terraform output bastion-public_dns) ssh -o StrictHostKeyChecking=no master.openshift.local master.openshift.local
	cat ./scripts/postinstall-node.sh | ssh -A ec2-user@$$(terraform output bastion-public_dns) ssh -o StrictHostKeyChecking=no master.openshift.local node1.openshift.local
	cat ./scripts/postinstall-node.sh | ssh -A ec2-user@$$(terraform output bastion-public_dns) ssh -o StrictHostKeyChecking=no master.openshift.local node2.openshift.local
